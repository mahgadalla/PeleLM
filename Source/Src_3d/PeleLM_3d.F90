...
!-----------------------------------  
  ...
  subroutine enth_diff_terms (lo, hi, dlo, dhi, dx, &
      ...
      REAL_T  rhoDz(DIMV(rhoDz),nspecies+2)
      integer DIMDEC(Fz)
...
!     actually be centered on the edge, so the stencils below must reflect this
      ...
      call amrex_allocate(H,T_l1,T_h1,T_l2,T_h2,T_l3,T_h3,1,nspecies)
...
!     Compute enthalpy flux as hi*(Fi+(lambda/cp).Grad(Yi))
      ...
      call amrex_allocate(rhoInv,lo(1)-1,hi(1)+1,lo(2)-1,hi(2)+1,lo(3)-1,hi(3)+1)
      ...
      do i=lo(1),hi(1)+1
         ...
         Fx(i,j,k,nspecies+2) = Fx(i,j,k,nspecies+2) &
             + (Fx(i,j,k,n) + rhoDx(i,j,k,nspecies+1)*gradY*Ax(i,j,k))*(H(i,j,k,n)+H(i-1,j,k,n))*0.5d0
      enddo
      enddo
      enddo
      enddo
      ...
      do i=lo(1),hi(1)
         ...
         Fy(i,j,k,nspecies+2) = Fy(i,j,k,nspecies+2) &
             + (Fy(i,j,k,n) + rhoDy(i,j,k,nspecies+1)*gradY*Ay(i,j,k))*(H(i,j,k,n)+H(i,j-1,k,n))*0.5d0
      enddo
      enddo
      enddo
      enddo
      ...
      do i=lo(1),hi(1)
         ...
         Fz(i,j,k,nspecies+2) = Fz(i,j,k,nspecies+2) &
             + (Fz(i,j,k,n) + rhoDz(i,j,k,nspecies+1)*gradY*Az(i,j,k))*(H(i,j,k,n)+H(i,j,k-1,n))*0.5d0
      enddo
      enddo
      enddo
      enddo
...
!     xlo
      if (fix_xlo) then
         ...
         do j=lo(2),hi(2)
            FiGHi(i,j,k) = FiGHi(i,j,k) - 0.5d0* &
                ( ( H(i+1,j,k,n) - H(i,j,k,n) )*Fx(i+1,j  ,k  ,n)*AD(2,i,j,k)     &
                + ( H(i,j,k,n) - H(i-1,j,k,n) )*Fx(i  ,j  ,k  ,n)*AD(1,i,j,k)*2.d0&
                + ( H(i,j+1,k,n) - H(i,j,k,n) )*Fy(i  ,j+1,k  ,n)*AD(4,i,j,k)     &
                + ( H(i,j,k,n) - H(i,j-1,k,n) )*Fy(i  ,j  ,k  ,n)*AD(3,i,j,k)     &
                + ( H(i,j,k+1,n) - H(i,j,k,n) )*Fz(i  ,j  ,k+1,n)*AD(6,i,j,k)     &
                + ( H(i,j,k,n) - H(i,j,k-1,n) )*Fz(i  ,j  ,k  ,n)*AD(5,i,j,k)     )
...
!     xhi
      if (fix_xhi) then
         ...
         do j=lo(2),hi(2)
            FiGHi(i,j,k) = FiGHi(i,j,k) - 0.5d0* &
                ( ( H(i+1,j,k,n) - H(i,j,k,n) )*Fx(i+1,j  ,k  ,n)*AD(2,i,j,k)*2.d0&
                + ( H(i,j,k,n) - H(i-1,j,k,n) )*Fx(i  ,j  ,k  ,n)*AD(1,i,j,k)     &
                + ( H(i,j+1,k,n) - H(i,j,k,n) )*Fy(i  ,j+1,k  ,n)*AD(4,i,j,k)     &
                + ( H(i,j,k,n) - H(i,j-1,k,n) )*Fy(i  ,j  ,k  ,n)*AD(3,i,j,k)     & 
                + ( H(i,j,k+1,n) - H(i,j,k,n) )*Fz(i  ,j  ,k+1,n)*AD(6,i,j,k)     &
      ...
      if (fix_ylo) then
         ...
         do i=lo(1),hi(1)
            FiGHi(i,j,k) = FiGHi(i,j,k) - 0.5d0* &
                ...
                + ( H(i,j,k,n) - H(i-1,j,k,n) )*Fx(i  ,j  ,k  ,n)*AD(1,i,j,k)     &
                + ( H(i,j+1,k,n) - H(i,j,k,n) )*Fy(i  ,j+1,k  ,n)*AD(4,i,j,k)     &
                + ( H(i,j,k,n) - H(i,j-1,k,n) )*Fy(i  ,j  ,k  ,n)*AD(3,i,j,k)*2.d0&
                + ( H(i,j,k+1,n) - H(i,j,k,n) )*Fz(i  ,j  ,k+1,n)*AD(6,i,j,k)     &
                + ( H(i,j,k,n) - H(i,j,k-1,n) )*Fz(i  ,j  ,k  ,n)*AD(5,i,j,k)     )
         enddo
         enddo
      ...
      if (fix_yhi) then
         ...
         do i=lo(1),hi(1)
            FiGHi(i,j,k) = FiGHi(i,j,k) - 0.5d0* &
                ( ( H(i+1,j,k,n) - H(i,j,k,n) )*Fx(i+1,j  ,k  ,n)*AD(2,i,j,k)     &
                + ( H(i,j,k,n) - H(i-1,j,k,n) )*Fx(i  ,j  ,k  ,n)*AD(1,i,j,k)     &
                + ( H(i,j+1,k,n) - H(i,j,k,n) )*Fy(i  ,j+1,k  ,n)*AD(4,i,j,k)*2.d0&
                + ( H(i,j,k,n) - H(i,j-1,k,n) )*Fy(i  ,j  ,k  ,n)*AD(3,i,j,k)     &
                + ( H(i,j,k+1,n) - H(i,j,k,n) )*Fz(i  ,j  ,k+1,n)*AD(6,i,j,k)     &
                + ( H(i,j,k,n) - H(i,j,k-1,n) )*Fz(i  ,j  ,k  ,n)*AD(5,i,j,k)     )
         enddo
      ...
      if (fix_zlo) then
         ...
         do i=lo(1),hi(1)
            FiGHi(i,j,k) = FiGHi(i,j,k) - 0.5d0* &
                ...
                + ( H(i,j,k,n) - H(i,j-1,k,n) )*Fy(i  ,j  ,k  ,n)*AD(3,i,j,k)     & 
                + ( H(i,j,k+1,n) - H(i,j,k,n) )*Fz(i  ,j  ,k+1,n)*AD(6,i,j,k)     &
                + ( H(i,j,k,n) - H(i,j,k-1,n) )*Fz(i  ,j  ,k  ,n)*AD(5,i,j,k)*2.d0)
         enddo
         enddo
         enddo
      endif
      ...
      if (fix_zhi) then
         ...
         do i=lo(1),hi(1)
            FiGHi(i,j,k) = FiGHi(i,j,k) - 0.5d0* &
                ...
                + ( H(i,j+1,k,n) - H(i,j,k,n) )*Fy(i  ,j+1,k  ,n)*AD(4,i,j,k)     &
                + ( H(i,j,k,n) - H(i,j-1,k,n) )*Fy(i  ,j  ,k  ,n)*AD(3,i,j,k)     &
                + ( H(i,j,k+1,n) - H(i,j,k,n) )*Fz(i  ,j  ,k+1,n)*AD(6,i,j,k)*2.d0&
                + ( H(i,j,k,n) - H(i,j,k-1,n) )*Fz(i  ,j  ,k  ,n)*AD(5,i,j,k)     )
         enddo
         enddo
         enddo
